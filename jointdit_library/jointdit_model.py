...
def joint_attention(q: Tensor, k: Tensor, v: Tensor, pe: Tensor, attn_mask: Optional[Tensor] = None, blend_source=None) -> Tensor:
    q, k = apply_rope(q, k, pe)

    batch_size = q.size(0)

    q_rgb = q[:batch_size // 2]
    q_dep = q[batch_size // 2:]
    
    k_rgb = k[:batch_size // 2]
    k_dep = k[batch_size // 2:]
    ...
    o_rgb = rearrange(o_rgb, "B H L D -> B L (H D)")
    o_dep = rearrange(o_dep, "B H L D -> B L (H D)")
...
class DoubleStreamBlock(nn.Module):
    ...
    def _forward(
        self, img: Tensor, txt: Tensor, vec: Tensor, pe: Tensor, txt_attention_mask: Optional[Tensor] = None, blend_source=None
    ) -> tuple[Tensor, Tensor]:
        ...
        # print("doublestream block investigate!")
        ...
        o_rgb, o_dep = joint_attention(q, k, v, pe=pe, attn_mask=attn_mask, blend_source=blend_source) 

        o_concat = torch.cat([o_rgb, o_dep], dim=-1)
        o_out = self.joint1(o_concat)
        o_rgb, o_dep = torch.chunk(o_out, 2, -1)
        
        t_rgb = blend_source[:blend_source.shape[0]//2]
        t_dep = blend_source[blend_source.shape[0]//2:]

        w_rgb = t_dep / (t_rgb + t_dep) 
        w_dep = t_rgb / (t_rgb + t_dep) 

        w_rgb = torch.sigmoid( 3 * (w_rgb - 0.5))
        w_dep = torch.sigmoid( 3 * (w_dep - 0.5))

        o_rgb = o_rgb * w_dep.unsqueeze(-1).unsqueeze(-1)
        o_dep = o_dep * w_rgb.unsqueeze(-1).unsqueeze(-1) 
            
        o_attn = torch.cat([o_rgb, o_dep], dim=0)
        attn = attn + o_attn
    ...
    def forward(
        ...
        if self.training and self.gradient_checkpointing:
            if not self.cpu_offload_checkpointing:
                return checkpoint(self._forward, img, txt, vec, pe, txt_attention_mask, blend_source, use_reentrant=False)
            ...
            return torch.utils.checkpoint.checkpoint(
                create_custom_forward(self._forward), img, txt, vec, pe, txt_attention_mask, blend_source, use_reentrant=False
            )

        else:
            return self._forward(img, txt, vec, pe, txt_attention_mask, blend_source)
...
class SingleStreamBlock(nn.Module):
    ...
    def _forward(self, x: Tensor, vec: Tensor, pe: Tensor, txt_attention_mask: Optional[Tensor] = None, blend_source=None) -> Tensor:
    ...
    def forward(self, x: Tensor, vec: Tensor, pe: Tensor, txt_attention_mask: Optional[Tensor] = None, blend_source=None) -> Tensor:
        if self.training and self.gradient_checkpointing:
            if not self.cpu_offload_checkpointing:
                return checkpoint(self._forward, x, vec, pe, txt_attention_mask, blend_source, use_reentrant=False)
            ...
            return torch.utils.checkpoint.checkpoint(
                create_custom_forward(self._forward), x, vec, pe, txt_attention_mask, blend_source, use_reentrant=False
            )
        else:
            return self._forward(x, vec, pe, txt_attention_mask, blend_source)
...
class JointDiT(nn.Module):
    ...
    def forward(
        ...
        blend_source = timesteps

        if not self.blocks_to_swap:
            for block_idx, block in enumerate(self.double_blocks):
                img, txt = block(img=img, txt=txt, vec=vec, pe=pe, txt_attention_mask=txt_attention_mask, blend_source=blend_source)
            ...
            for block_idx, block in enumerate(self.single_blocks):
                img = block(img, vec=vec, pe=pe, txt_attention_mask=txt_attention_mask, blend_source=blend_source)
